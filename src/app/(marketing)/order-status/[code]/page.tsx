...
import { CheckCircle2, Clock, Truck, PackageCheck, XCircle, ArrowLeft, RefreshCw, AlertCircle } from "lucide-react";
...
export default function OrderStatusPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-surface-container-lowest py-24 px-6 flex flex-col items-center justify-center">
        ...
        <p className="text-on-surface-variant font-medium text-sm">Đang tra cứu thông tin đơn hàng {code}...</p>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-surface-container-lowest py-24 px-6 flex items-center justify-center">
        <div className="max-w-md w-full bg-white p-8 rounded-3xl border border-outline-variant/30 shadow-xl text-center space-y-6">
          ...
          <h2 className="text-headline-md text-primary font-bold">Không tìm thấy đơn hàng</h2>
          <p className="text-sm text-on-surface-variant leading-relaxed">
            {error || `Chúng tôi không thể tìm thấy đơn hàng nào khớp với mã "${code}". Vui lòng kiểm tra lại mã đơn hàng hoặc liên hệ CSKH.`}
          </p>
  ...
  const steps = [
    { key: "PENDING", label: "Chờ xác nhận", desc: "Đang kiểm tra đối soát CK", icon: Clock },
    { key: "CONFIRMED", label: "Đã xác nhận", desc: "Đơn hàng thành công", icon: CheckCircle2 },
    { key: "SHIPPED", label: "Đang vận chuyển", desc: "Đang trên đường giao", icon: Truck },
    { key: "DELIVERED", label: "Đã giao hàng", desc: "Hoàn tất giao hàng", icon: PackageCheck },
  ];
      ...
      <div className="max-w-4xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-outline-variant/20 pb-6">
          <div>
            ...
            <h1 className="text-3xl font-bold text-on-surface flex items-center gap-3">
              Đơn hàng <span className="text-primary font-mono bg-primary/5 px-3 py-1 rounded-xl border border-primary/10">{order.orderCode}</span>
            </h1>
            <p className="text-xs text-on-surface-variant mt-1">Đặt lúc: {new Date(order.createdAt).toLocaleString("vi-VN")}</p>
          </div>
        ...
        <div className="bg-white p-8 rounded-3xl border border-outline-variant/30 shadow-xs">
          ...
          {isCancelled ? (
            <div className="flex items-center justify-center gap-3 p-6 bg-red-50 text-red-700 rounded-2xl border border-red-200 font-semibold">
              ...
              <span>Đơn hàng này đã bị hủy. ({order.adminNote || "Lý do: Không nhận được thanh toán hoặc theo yêu cầu"})</span>
            </div>
          ) : (
          ...
          {order.status === "PENDING" && !isCancelled && (
            <div className="mt-8 p-4 bg-amber-50 rounded-2xl border border-amber-200 flex items-start gap-3 text-amber-900 text-xs leading-relaxed">
              ...
              <div>
                <strong className="font-bold text-amber-950 block mb-0.5">Trạng thái: Đang chờ đối soát thanh toán</strong>
                Quản trị viên đang kiểm tra giao dịch thanh toán qua mã QR với nội dung <strong className="font-mono text-amber-950">{order.orderCode}</strong>. Quá trình này thường mất từ 5-15 phút. Khi xác nhận khớp giao dịch, đơn hàng sẽ chuyển sang trạng thái <strong>Đã xác nhận (Thành công)</strong>.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-3xl border border-outline-variant/30 shadow-xs space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <span className="text-on-surface-variant block mb-0.5 uppercase text-[10px]">Người nhận:</span>
                <span className="font-bold text-sm text-on-surface">{order.customerName}</span>
              </div>
              <div>
                <span className="text-on-surface-variant block mb-0.5 uppercase text-[10px]">Số điện thoại:</span>
                <span className="font-bold text-sm text-on-surface font-mono">{order.customerPhone}</span>
              </div>
              {order.customerEmail && (
                <div>
                  <span className="text-on-surface-variant block mb-0.5 uppercase text-[10px]">Email:</span>
                  <span className="font-medium text-sm text-on-surface">{order.customerEmail}</span>
                </div>
              )}
              <div>
                <span className="text-on-surface-variant block mb-0.5 uppercase text-[10px]">Địa chỉ giao hàng:</span>
                <span className="font-medium text-sm text-on-surface leading-normal">{order.shippingAddress}</span>
              </div>
              <div>
                <span className="text-on-surface-variant block mb-0.5 uppercase text-[10px]">Phương thức thanh toán:</span>
                <span className="inline-flex items-center gap-1 font-bold text-primary bg-primary/5 px-2.5 py-1 rounded-lg border border-primary/10">
                  {order.paymentMethod === "VIETQR" ? "Thanh toán qua mã QR" : order.paymentMethod}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-outline-variant/30 shadow-xs flex flex-col justify-between">
                  ...
                  <div key={index} className="flex gap-4 pt-3 first:pt-0">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-0.5">
                      <div>
                        ...
                        {item.sku && <p className="text-[11px] text-on-surface-variant font-mono mt-0.5">SKU: {item.sku}</p>}
                      </div>
                      <div className="flex justify-between items-center text-xs font-semibold">
                        <span className="text-on-surface-variant">Số lượng: x{item.quantity}</span>
                        <span className="text-primary font-bold">{formatCurrencyVND(item.price)}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="border-t border-outline-variant/20 pt-4 mt-6 space-y-2">
              ...
              <div className="flex justify-between items-center text-base font-bold text-on-surface pt-1">
                ...
                <span className="text-xl text-primary font-extrabold">{formatCurrencyVND(order.totalAmount)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-container-low p-6 rounded-2xl border border-outline-variant/30 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-sm text-on-surface-variant font-medium">
          <div>
            ...
            <p className="text-xs">Hotline hỗ trợ khách hàng hoạt động 24/7 để giải đáp mọi thắc mắc.</p>
          </div>
          <Link href="/contact" className="px-5 py-2.5 bg-secondary text-white rounded-xl hover:bg-secondary/90 transition-all font-semibold tracking-wide text-xs whitespace-nowrap text-center shadow-xs">
            LIÊN HỆ CSKH
          </Link>
        </div>
      </div>
