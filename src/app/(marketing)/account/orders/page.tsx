...
import { Clock, CheckCircle2, Truck, PackageCheck, XCircle, ArrowRight, PackageOpen, ShoppingBag } from "lucide-react";
    ...
    switch (status) {
      ...
      case "CONFIRMED":
        return {
          ...
          class: "bg-emerald-50 text-emerald-700 border-emerald-300 font-semibold",
          icon: CheckCircle2,
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-surface-container-lowest py-24 px-6 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-on-surface-variant font-medium text-sm">Đang tải lịch sử đơn hàng của bạn...</p>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-surface-container-lowest py-24 px-6 flex items-center justify-center">
        <div className="max-w-md w-full bg-white p-8 rounded-3xl border border-outline-variant/30 shadow-xl text-center space-y-6">
          ...
          <h2 className="text-headline-md text-primary font-bold">Lỗi truy xuất đơn hàng</h2>
          <p className="text-sm text-on-surface-variant leading-relaxed">
            {error}. Vui lòng kiểm tra lại trạng thái đăng nhập hoặc liên hệ CSKH.
          </p>
      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-12">
        <div className="flex items-center justify-between border-b border-outline-variant/20 pb-6">
          <div>
            ...
            <p className="text-xs text-on-surface-variant mt-1 font-medium">Quản lý và theo dõi trạng thái các đơn hàng bạn đã đặt mua</p>
          </div>
        ...
        {orders.length === 0 ? (
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-on-surface">Chưa có đơn hàng nào</h2>
            <p className="text-sm text-on-surface-variant max-w-md mx-auto leading-relaxed">
              Bạn chưa thực hiện bất kỳ giao dịch mua sắm nào. Khám phá các sản phẩm nhân sâm tự nhiên cao cấp của chúng tôi ngay hôm nay!
            </p>
            <div className="pt-4">
              <Link
                href="/products"
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-6 border-b border-outline-variant/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <span className="font-mono text-lg font-bold text-primary">#{order.orderCode}</span>
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs border shadow-xs ${statusDisplay.class}`}>
                          <StatusIcon size={14} className={statusDisplay.color} />
                          {statusDisplay.label}
                        </span>
                      </div>
                      <span className="text-xs text-on-surface-variant">Ngày đặt: {new Date(order.createdAt).toLocaleString("vi-VN")}</span>
                    </div>

                    <button
                      onClick={() => router.push(`/order-status/${order.orderCode}`)}
                    ...
                    {order.items.map((it, idx) => (
                      <div key={idx} className="flex items-center justify-between py-4 first:pt-0 last:pb-0 gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="font-bold text-sm text-on-surface line-clamp-1">{it.name}</h4>
                            {it.sku && <p className="text-[11px] text-on-surface-variant font-mono mt-0.5">SKU: {it.sku}</p>}
                          </div>
                        </div>

                        <div className="flex items-center gap-6 text-xs">
                          <span className="font-semibold text-on-surface-variant">x{it.quantity}</span>
                          <span className="font-bold text-primary font-mono sm:text-sm">{formatCurrencyVND(it.price)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
