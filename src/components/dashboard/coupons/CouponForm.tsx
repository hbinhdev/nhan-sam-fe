...
function normalizeMoneyValueForInput(
  value: number | string | null | undefined,
) {
  if (value === null || value === undefined || value === "") {
    return "";
...
export function CouponForm({ mode, initialData }: CouponFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    discountType: (initialData?.discountType ||
      "PERCENTAGE") as CouponDiscountType,
    ...
    minOrderAmount: normalizeMoneyValueForInput(
      initialData?.minOrderAmount ?? 0,
    ),
    maxDiscountAmount:
      initialData?.maxDiscountAmount !== null &&
      initialData?.maxDiscountAmount !== undefined
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : "Không thể lưu mã giảm giá.",
        "error",
      );
    } finally {
      setSaving(false);
            ...
            <Field label="Giá trị giảm *">
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500">
                  {form.discountType === "FIXED_AMOUNT"
                    ? `≈ ${formatCurrencyVND(
                        Number.isFinite(previewDiscountValue)
          ...
          <details className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field label="Giá trị đơn hàng tối thiểu">
                ...
                />
                <p className="text-xs text-slate-500">
                  {formatCurrencyVND(Number(form.minOrderAmount || 0))}
                </p>
              </Field>
              ...
              <Field label="Mức giảm tối đa">
                ...
                />
                <p className="text-xs text-slate-500">
                  {formatCurrencyVND(Number(form.maxDiscountAmount || 0))}
                </p>
              </Field>
